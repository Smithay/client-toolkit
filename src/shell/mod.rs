...
pub enum Shell {
    /// The current standart `xdg_shell` protocol.
    ...
    /// The legacy `wl_shell`.
    ///
    /// It is deprecated and only present here for compability purposes.
...
// An utility for lazy-loading globals, as for
// shells we'll only want to load one of them in general
enum Lazy<I: Interface> {
    Unknown,
...
struct ShellInner {
    ...
    wl_shell: Lazy<wl_shell::WlShell>,
    xdg_shell: Lazy<xdg_wm_base::XdgWmBase>,
    zxdg_shell: Lazy<zxdg_shell_v6::ZxdgShellV6>,
}
...
impl GlobalHandler<wl_shell::WlShell> for ShellHandler {
    ...
    fn get(&self) -> Option<Attached<wl_shell::WlShell>> {
        ...
        match inner.wl_shell {
            Lazy::Bound(ref shell) => Some(shell.clone()),
            Lazy::Unknown => None,
            Lazy::Seen { id, .. } => {
                ...
                inner.wl_shell = Lazy::Bound((*shell).clone());
                Some((*shell).clone())
            }
        }
    }
    ...
    fn get(&self) -> Option<Attached<xdg_wm_base::XdgWmBase>> {
        ...
        match inner.xdg_shell {
            ...
            Lazy::Seen { version, id } => {
                ...
                inner.xdg_shell = Lazy::Bound((*shell).clone());
                Some((*shell).clone())
            }
        }
    }
    ...
    fn get(&self) -> Option<Attached<zxdg_shell_v6::ZxdgShellV6>> {
        ...
        match inner.zxdg_shell {
            ...
            Lazy::Seen { id, .. } => {
                ...
                inner.zxdg_shell = Lazy::Bound((*shell).clone());
                Some((*shell).clone())
            }
        }
    }
...
impl<E: ShellHandling> Environment<E> {
    ...
    /// and `wl_shell`) for retrocompatibility. It'll attempt to use them in this order.
    ///
    /// You need to provide a closure that will process the events generated by the shell surface.
    ///
    /// *Panic*
    ///
    /// This function will panic if no supported shell was advertized by the compositor.
